//
// Created by sarsa on 24-06-2025.
//

// things are left to be implemented here like a function to get corners

#include "GenericRubiksCubeModel.h"

#define ull uint64_t

class RubiksCubeBitboard : public GenericRubiksCubeModel {
private:
...
public:
    ...
    RubiksCubeBitboard() {
        for (int side =0; side < 6; side++) {
            bitboard[side] = 0;
            uint64_t  clr = 1<<side;
            for (int sticker_index =0; sticker_index < 8 ; sticker_index++) {
                bitboard[side] |= clr << (sticker_index*8);
            }
    ...
    COLOR getColor(FACE face, unsigned row, unsigned col) const override {
        ...
        uint64_t clr = (side>>(8*idx))&one_8;
        int dist_of_one_from_left=0;
        while (clr!=0) {
            clr>>=1;
            dist_of_one_from_left++;
        }
        return (COLOR)(dist_of_one_from_left - 1);
    }
